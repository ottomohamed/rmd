...
function PageLoader() {
  return (
    <div className="min-h-[50vh] flex flex-col items-center justify-center gap-4" dir="rtl">
      ...
      <p className="text-sm text-gray-400 font-bold tracking-wider animate-pulse">جاري التحميل...</p>
    </div>
  );
}
  ...
  return (
    <Layout>
      <ErrorBoundary fallback={
        <div className="p-10 text-center font-sans bg-red-50 dark:bg-red-950/20 rounded-xl border border-red-100 dark:border-red-900/30 m-4" dir="rtl">
          <h2 className="text-xl font-bold mb-2 text-red-600 dark:text-red-400">عذراً، حدث خطأ تقني</h2>
          <p className="text-red-500/70 text-sm mb-4">يرجى تحديث الصفحة أو المحاولة لاحقاً.</p>
          <button onClick={() => window.location.reload()} className="px-6 py-2 bg-red-600 text-white rounded-lg text-sm font-bold hover:bg-red-700 transition-colors">
            تحديث الصفحة
          </button>
        </div>
      }>
          ...
          <Switch>
            ...
            <Route>
              <div className="flex flex-col items-center justify-center min-h-[60vh] text-center" dir="rtl">
                ...
                <p className="text-gray-400 font-sans text-lg mb-8">المقال الذي تبحث عنه غير موجود حالياً.</p>
                <a href="/" className="px-8 py-3 bg-emerald-700 text-white rounded-xl font-bold text-sm hover:bg-emerald-800 transition-all">
                  العودة للرئيسية
                </a>
              </div>
            </Route>
