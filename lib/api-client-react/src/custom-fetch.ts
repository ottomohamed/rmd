...
export function setApiLang(lang: string) {
  _apiLang = (lang ?? "en").split("-")[0].toLowerCase();
}
export function getApiLang() { return _apiLang; }

const LANG_ENDPOINTS = ["/api/products", "/api/search"];

function appendLangParam(url: string): string {
  if (!LANG_ENDPOINTS.some((p) => url.includes(p))) return url;
  const sep = url.includes("?") ? "&" : "?";
  return `${url}${sep}lang=${_apiLang}`;
}
...
export async function customFetch<T = unknown>(
  ...
  const resolvedUrl = appendLangParam(resolveUrl(input));
